...
// GET /api/transactions?member=&q=&group=&asset=&type=
export async function onRequestGet({ env, request }) {
  try {
    ...
    if (asset)  { where.push('t.asset_id = ?');  params.push(Number(asset)); }
    if (type)   { where.push('t.type = ?');      params.push(String(type)); }
    if (q)      { where.push('(a.name LIKE ? OR t.notes LIKE ?)'); params.push(`%${q}%`, `%${q}%`); }

    const sql = `
      SELECT t.*, a.name AS asset_name, a.group_id AS asset_group, m.name AS member_name
      FROM transactions t
...
export async function onRequestPost({ env, request }) {
  try {
    ...
    const total     = b.total != null && b.total !== '' ? Number(b.total) : (qty * unitPrice);

    const result = await env.DB.prepare(`
      INSERT INTO transactions (date, type, asset_id, member_id, qty, unit_price, fee, tax, total, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      b.date,
      b.type,
